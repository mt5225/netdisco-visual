  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.color))
...
#        console.log "link: #{l.source.id},#{l.target.id}\n"
    data
...
#      console.log "Nodes: #{n.id}\n"
  ...
  filterNodes = (allNodes) ->
    ...
    #console.log(filter)
    if filter == "unreachable" or filter == "nofts"
        filteredNodes = allNodes.filter (n) ->
            if filter == "unreachable" and n.type != "node"
                n
            else if filter == "nofts" and n.color == "orange" and n.type == "device"
                n
  ...
  sortedHosts = (nodes,links) ->
    ...
    if sort == "links"
      ...
      hosts = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      hosts = hosts.map (v) -> v.key
    else
  ...
  updateText = () ->
    text = nodesG.selectAll("text.anchor")
    ...
    text.on("mouseover", showDetails)
      ...
      .on("mousedown", redirectDetails)
      #.attr("transform", (d) -> "rotate(" + (d.x + (d.x <= 270 && d.x >= 90 ? -180 : 0)) + ")translate(" + toffset(d) + ")")
    text.exit().remove()
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.color))
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.color)).darker().toString()
  ...
  redirectDetails = (d,i) ->
    console.log(d.url)
    window.open("http://MY_NETDISCO_SERVER/netdisco/"+d.url)
